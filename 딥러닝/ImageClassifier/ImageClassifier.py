...
def build_MLP_model(self):
    input_layer = tf.keras.layers.Input(
        shape=[self.img_shape_x, self.img_shape_y, ])
    flatten_layer = tf.keras.layers.Flatten()(input_layer)

    ac_func_relu = tf.keras.activations.relu
    hidden_layer_1 = tf.keras.layers.Dense(
        units=128, activation=ac_func_relu)(flatten_layer)
    hidden_layer_2 = tf.keras.layers.Dense(
        units=128, activation=ac_func_relu)(hidden_layer_1)

    ac_func_softmax = tf.keras.activations.softmax
    output_layer = tf.keras.layers.Dense(
        units=self.num_labels, activation=ac_func_softmax)(hidden_layer_2)
    ...
    classifier_model.compile(
        optimizer=opt_alg, loss=loss_cross_e, metrics=['accuracy'])
